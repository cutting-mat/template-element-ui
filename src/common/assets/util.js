...
export const getFullArray = function(val, array) {
    if (val && Array.isArray(array) && array.length) {
        ...
        const nodeIndex = targetData.findIndex(e => e.id == val)
        if (nodeIndex !== -1) {
...
export const optionsValue = (value, options, config) => {
        ...
        if (Array.isArray(array)) {
            return array.map(e => {
                return {
                    id: e.id,
                    pid: e.pid,
                    value: e[mergeConfig.value] || "",
                    label: e[mergeConfig.label] || "",
    ...
    const optionsData = trans(options);
    ...
    if (mergeConfig.deep) {
        ...
        if (Array.isArray(value)) {
            ...
            __ = result.join(mergeConfig.split);
        } else if (value !== void 0) {
            const targetIndex = optionsData.findIndex(e => {
                return e.value === value;
            });
            if (targetIndex !== -1) {
                __ = optionsData[targetIndex].label;
            } else {
                __ = "";
            }
        }
        ...
        // 一维数据结构
        return getFullArray(value, optionsData).map(e => e.label).join(mergeConfig.split)
    }
}